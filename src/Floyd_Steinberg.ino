...
TFT_eSprite frame =
    TFT_eSprite(&glc); // Invoke the Sprite class for the image frame buffer
...
uint16_t epd_height =
    EPD_HEIGHT; // in different landscape/portrait rotations
...
void setup() {
  ...
  frame.setColorDepth(
      1); // Must set the bits per pixel to 1 for ePaper displays
  ...
  // RAM needed is about (EPD_WIDTH * EPD_HEIGHT)/8 , ~5000 bytes for 200 x 200
  // pixels Note: always create the Sprite before setting the Sprite rotation
...
void loop() {
  frame.setRotation(
      random(4)); // Set the rotation to 0, 1, 2 or 3 ( 1 & 3 = landscape)
  ...
  frame.drawCircle(frame.width() / 2, frame.height() / 2, frame.width() / 6,
                   INK);
...
//------------------------------------------------------------------------------------
// setRotation() actually rotates the drawing coordinates, not the whole display
// frame buffer so we can use this to draw text at right angles or upside down
//------------------------------------------------------------------------------------
void rotateTest(void) {
  // frame.fillSprite(PAPER);             // Fill buffer with white to clear old
  // graphics
  ...
  frame.setRotation(
      0); // Set the display rotation to 0, 1, 2 or 3 ( 1 & 3 = landscape)
